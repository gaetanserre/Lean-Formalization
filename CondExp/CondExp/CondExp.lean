...
lemma constant_ae_eq_imp_eq {α β : Type*} [MeasurableSpace α] {μ : Measure α}
  ...
  have : μ ({(_ : α) | c = d} ∪ {(_ : α) | c = d}ᶜ) = 0 := by
    have : μ {(_ : α) | c = d} = 0 := by
      have : {(_ : α) | c = d} = ∅ := subset_eq_empty (fun _ ↦ h) rfl
      rw [this]
...
theorem probability_total_law :
  ...
  exact (setLIntegral_one s).symm

variable [IsProbabilityMeasure ν]

omit [Nonempty Ω] [StandardBorelSpace Ω] [IsFiniteMeasure μ] in
...
lemma apply_comp : μ.prod ν = μ ⊗ₘ (μ.prod ν).condKernel := by
  ...
  exact ((μ.prod ν).disintegrate (μ.prod ν).condKernel).symm
...
-/
lemma condition_m_trivial {X : Ω → F} (hm₁ : m₁ ≤ mΩ)
...
-/
theorem total_expectation_law {X : Ω → F} (hX : Integrable X μ)
  ...
  have total_exp : ∫ x, (μ[A.indicator fun _ ↦ (1 : ℝ)|gen_sigma Y]) x ∂μ
    ...
    have hI : HasFiniteIntegral (A.indicator (fun (_ : Ω) ↦ (1 : ℝ))) μ := by
      ...
      have rmv_norm : ∀ a, ‖A.indicator (fun (_ : Ω) ↦ (1 : ℝ)) a‖₊ =
        ...
        rfl
      simp_rw [rmv_norm]
      calc ∫⁻ a, A.indicator 1 a ∂μ = ∫⁻ _ in A, 1 ∂μ :=
        lintegral_indicator hAmΩ 1
